...
var ArticleTableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#articleTable').bootstrapTable({
            url: HOST + COMMON.NONPUBLIC_PREFIX + API.ARTICLE + "/list",         //请求后台的URL（*）
            ajaxOptions:{
                headers:{
            ...
            columns: [{
                ...
                formatter: function(value, row, index) {
                    ...
                    return '<a class="btn active btn-sm btn-primary" href="../article.html?id='+value+'">查看</a>' +
                        '<a class="btn active btn-sm btn-info" href="write-articles.html?id='+value+'">编辑</a>' +
                        '<a class="btn active btn-sm btn-warning" href="javascript:passArticle(\''+value+'\',\''+row.hasPass+'\')">'+pass+'</a>'+
                        '<a class="btn active btn-sm btn-danger" href="javascript:deleteArticle(\''+value+'\')">删除</a>';
                }
            } ]
        });
    };
...
function deleteArticle(id) {
    systemConfirm("确定删除该文章？",function () {
        $.ajax({
            url:HOST + COMMON.NONPUBLIC_PREFIX + API.ARTICLE + "/delete",
            type:"delete",
            headers:{
                token:$.cookie(USER_DIGEST_COOKIE)
            },
 ...
 * @param id
 */
function passArticle(id,hasPass) {
    $.ajax({
        url:HOST + COMMON.NONPUBLIC_PREFIX + API.ARTICLE + "/update_pass_status",
        type:"put",
        headers:{
            token:$.cookie(USER_DIGEST_COOKIE)
        },
